...
If you're using sge, you also need to specify the `--sge_q` and `--sge_pe` options.

The `--sge_q` option is the queue you want to use, and the `--sge_pe` option is the parallel environment you want to use. This is the value you would put where the word "smp" is in the example below.

`#$ -pe smp 30`

So in in this example, you would add --sge_pe = "smp".
