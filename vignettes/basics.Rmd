...
If you are in a HPC environment and you need to specify other options for a batch job that are outside of the job header, you can add them to the other_batch_options list. This is typically used for loading modules or conda environments, but you can put anything that is required in this vector. These commands will be run at the top of the batch job.


If you want to test everything out, try running get_snp_tree with temp_dir set to an easy to access directory and submit = FALSE. This will generate the batch job scripts, but not submit them. Then you can look at the scripts to see if they look correct. You can also run the scripts manually to see if they work and what options you may need to add.
